...
export class TuneInComponent implements AfterViewInit, OnInit {
  ...
  ngOnInit() {
    ...
    this.regionGroupOptions = this.formGroup
      .get('nationality')!
      .valueChanges.pipe(
        startWith(''),
  ...
  private _filterGroup(value: string): any[] {
    if (value) {
      return this.regions
        .map((region) => ({
          name: region.name,
          countries: _filter(region.countries, value),
        }))
  ...
  onSubmit() {
    ...
    const queryText =
      ...
      `,\\"` +
      `texto8\\":\\"` +
      this.formGroup.get('nationality').value +
      `\\"` +
      `,\\"` +
      `texto9\\":\\"` +
    ...
    this.http
      .post('https://api.monday.com/v2', this.formDataText, {
        headers: reqHeader,
      })
      .subscribe(
        (resText: any) => {
          this.formData.forEach((field) => {
            let data = {};
            data['id_item'] = resText.data.create_item.id;
